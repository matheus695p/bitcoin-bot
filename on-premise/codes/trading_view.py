    ...
    with open('data_file.csv', mode='w', newline='') as data_file:
        ...
        for xi in x:
            ...
            ts = datetime.fromtimestamp(
                float(xi[4])).strftime("%Y/%m/%d, %H:%M:%S")
            employee_writer.writerow([ind, ts, float(xi[5]), float(
                xi[6]), float(xi[7]), float(xi[8]), float(xi[9])])


def generate_csv_ethereum(a):
    out = re.search('"s":\[(.+?)\}\]', a).group(1)
    x = out.split(',{\"')
...
sendMessage(ws, "quote_add_symbols", [
            session, f"BINANCE:{crypto}", {"flags": ['force_permission']}])

sendMessage(ws, "resolve_symbol", [chart_session, "symbol_1",
            "={\"symbol\":\"BINANCE:{crypto}\",\"adjustment\":\"splits\"}"])
sendMessage(ws, "create_series", [
            chart_session, "s1", "s1", "symbol_1", "1", 300])

# sendMessage(ws, "quote_fast_symbols", [session, "BINANCE:BTCUSDT"])

sendMessage(ws, "quote_fast_symbols", [session, f"BINANCE:{crypto}"])

sendMessage(ws, "create_study", [chart_session, "st1", "st1", "s1",
            "Volume@tv-basicstudies-118", {"length": 20, "col_prev_close": "false"}])
